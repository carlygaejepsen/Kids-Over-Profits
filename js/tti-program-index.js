...
(function() {
    // Global configuration - safely access globals injected by WP or inline scripts
    const localFacilitiesConfig = window.facilitiesConfig || {};
    const localTtiIndexConfig = window.ttiIndexConfig || { isAdmin: false };
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Load Data
        const load = async () => {
            if (localFacilitiesConfig.jsonDataUrl) {
                try {
                    const res = await fetch(localFacilitiesConfig.jsonDataUrl);
                    if (res.ok) {
