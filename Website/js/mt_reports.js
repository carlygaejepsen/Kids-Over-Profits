        ...
        const facilities = reports.reduce((acc, report) => {
            ...
            if (!acc[facilityId]) {
                acc[facilityId] = {
                    ...
                    capacity: 'N/A', // Montana data doesn't seem to have capacity
                    city: report.Header.City || '',
                    address: report.Header['Physical Address'] || '',
                    zipCode: report.Header['Zip Code'] || '',
            ...
            const inspection = {
                ...
                deficiencies: report.Issues.map(issue => ({
                    ...
                    plan_of_correction: issue['Plan of Correction'] || '',
                    item_number: issue['Item #'] || '',
                    is_repeat: issue['Repeat Deficiency'] === 'Yes'
                })) || []
            };

            acc[facilityId].inspections.push(inspection);
            return acc;
        }, {});
            ...
            facilityElement.innerHTML = `
                <details>
                    <summary class="facility-header">
                        ...
                        <h2>License Number: ${facility.number || 'N/A'}</h2>
                        <p class="facility-details">Type: ${facility.facility_type || 'N/A'} | Administrator: ${toTitleCase(facility.officer) || 'N/A'} | City: ${facility.city || 'N/A'}</p>
                        ${facility.address ? `<p class="facility-details">Address: ${facility.address}${facility.zipCode ? `, ${facility.zipCode}` : ''}</p>` : ''}
                        ${facility.phone ? `<p class="facility-details">Phone: ${facility.phone}</p>` : ''}
                    </summary>
    ...
    function createDeficiencyHTML(deficiency, index) {
        ...
        return `
            <details class="violation-box">
                <summary class="deficiency-header">Deficiency ${index + 1}${itemNumber}${repeatFlag}</summary>
                <div class="deficiency-content">
                    <strong>Rule Cited:</strong><p>${section || 'N/A'}</p>
                    <strong>Findings:</strong><p>${description || 'N/A'}</p>
